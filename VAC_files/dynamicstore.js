	...
	UpdateDynamicBundleElements: function( Bundle, $El )
	{
		...
		else if ( !Bundle.m_bMustPurchaseAsSet )
		{
			...
			if ( !Bundle.m_bContainsDiscountedPackage )
			{
				...
				$DiscountBlocks.find('.discount_final_price' ).addClass('your_price' ).empty().append($J('<div/>', {'class': 'your_price_label'} ).text('您的定价：'), $J('<div/>' ).text( strFormattedFinalPrice ) );
			}
			else
			{
	...
	PopulateRecommendedTagList: function()
	{
		...
		for( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length && i < 5; i++ )
		{
			var tag = GDynamicStore.s_rgRecommendedTags[i];
							var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'https://store.steampowered.com/tags/zh-cn/' + encodeURIComponent( tag.name ) });
						$Link.text( tag.name );
			$Element.append( $Link );
		}
	},
...
{
	return "https:\/\/media.st.dl.bscstorage.net\/steamcommunity\/public\/images\/avatars\/" + strHash.substring( 0, 2 ) + '/' + strHash + strSize + '.jpg';
}

function GetScreenshotURL( appid, filename, sizeStr )
{
	if( sizeStr )
		return 'https://media.st.dl.bscstorage.net/steam/' + 'apps/' + appid + '/' + filename.replace('.jpg', sizeStr + '.jpg');

	return 'https://media.st.dl.bscstorage.net/steam/' + 'apps/' + appid + '/' + filename;
}
...
function UpdateStoreBannerForAdditionalCartDiscount( nCartDiscount )
{
	...
	<div class="placeHolder_lunarSale2019_giftActiveBar">	\
		...
		<div class="lunar_sale_title"><img src="https://store.st.dl.bscstorage.net/public/images/promo/lunar2019/lny2019_title_zh-cn.png"/></div> \
